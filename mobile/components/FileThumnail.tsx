...
export default function FileThumbnail({data, onSelect}) {
  ...
  const fetchThumbnail = async () => {
    try {
      let path2 = FileSystem.documentDirectory;
      if (data.folder_id) {
        try {
          const data2 = await getFolderInfo(data.folder_id);
          path2 = data2.path;
        } catch (error) {
          console.error(error);
          return;
      ...
      const thumb = await Thumb.getThumbnailAsync(path2 + data.file_name, {
        time: 15000,
      });
  ...
  return (
    <Pressable onPress={handleClick} style={{width: '33%'}}>
      <View
        ...
        {thumbnail && (
          <>
            <Image source={thumbnail} style={{width: 100, height: 70}} />
            {data.extension && (
              <View
                style={{
                  position: 'absolute',
        ...
        {data.extension ? (
          <>
            <Text>
              {data.name.substring(0, 25)}.{data.extension}
            </Text>
            <Text>{data.date}</Text>
