...
export default function MediaPlayer({uri, onClose}: MediaPlayerProps) {
  ...
  const mooveAnimation = (
    ...
    if (deltaY >= 0) {
      ...
      newHeight =
        initialH - deltaY * 1.45 >= windowHeight
          ? windowHeight
          : initialH - deltaY * 1.45;
      newWidth =
        initialW - deltaX >= windowWidth ? windowWidth : initialW - deltaX;
    }
    ...
    if (direction === 'vertical') {
      if (
        (initialH >= 710 && gestureY < 0) ||
        (initialH <= 100 && gestureY > 0)
      ) {
        return;
      }
    ...
    PanResponder.create({
      ...
      onPanResponderMove: (_, gestureState) => {
        ...
        if (isBottom.current) {
          ...
          if (
            (initialY >= 710 && gestureState.dy > 0) ||
            (initialHeight <= 100 && gestureState.dy > 0)
          ) {
            return;
          }
  ...
  return (
    <Animated.View
      ...
      <View
        {...panResponder.panHandlers}
        style={{width: '100%', height: '29%', minHeight: 100, zIndex: 1}}>
        <Video
          ref={videoRef}
          source={{uri: uri}}
          rate={1.0}
